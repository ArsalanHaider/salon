<?php if($this->uri->segment(2) != "manage_forms" && $this->uri->segment(2) !="edit_form" ) { ?>
    <script>
    function format ( d ) {
    ...
    $("#phase").change(function(){
        let phase_id = $("#phase").val();
        
        $.get("<?php echo base_url('phase/get_plots/'); ?>" + phase_id, function(data){
            data = JSON.parse(data);
            let options = "<option> Please select Plot</option>";
    ...
    $(document).ready(function() {
        $('#allottees').DataTable( {
            ...
            // Initial no order.
             ...
             "order": [[0, "asc" ]],
             "ajax":{
                  url :  '<?php echo base_url('allottee/get_form_data'); ?>',
                  type : 'POST'
                },
        }); // End of DataTable
